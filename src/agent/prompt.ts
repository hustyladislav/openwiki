...
- OpenWiki has built-in local connectors for git-repo, notion, x, google, web-search, hackernews, slack, and langsmith. Use openwiki_list_connectors to inspect connector capabilities, config paths, required env var names, and raw data paths.
...
- For LangSmith, use deterministic direct API ingestion through openwiki_ingest_connector with connectorId "langsmith". It exhaustively paginates a fixed root-run time window, groups compact user/assistant evidence by thread, records trace provenance, and advances its per-instance cursor only after the complete window is written. Read manifest.json first and then only the relevant thread files. Do not treat agent hypotheses, tool payloads, system prompts, or model reasoning as user-authored memory.
